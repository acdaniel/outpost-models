...
function buildPropertySchema (name, propertyDef) {
  ...
  switch (propertyDef.type.trim().toLowerCase()) {
    ...
    case 'string':
      ...
      break;
    case 'objectid':
      schema = [joi.string(), joi.object().type(bson.ObjectId)];
      break;
    case 'any':
    case '*':
      schema = joi.any();
  ...
  for (var i = 0, l = schema.length; i < l; i++) {
    if (propertyDef.default) { schema[i].default(propertyDef.default); }
    if (propertyDef.min) { schema[i].min(propertyDef.min); }
    if (propertyDef.max) { schema[i].max(propertyDef.max); }
    if (propertyDef.length) { schema[i].length(propertyDef.length); }
    if (propertyDef.allow) { schema[i].allow(propertyDef.allow); }
    if (propertyDef.valid) { schema[i].valid(propertyDef.valid); }
    if (propertyDef.invalid) { schema[i].invalid(propertyDef.invalid); }
    if (propertyDef.forbidden) { schema[i].forbidden(); }
    if (propertyDef.strip) { schema[i].strip(); }
    if (propertyDef.required) { schema[i].required(); }
    if (propertyDef.strict) { schema[i].strict(propertyDef.strict); }
    if (propertyDef.label) { schema[i].label(propertyDef.label); }
    if (propertyDef.raw) { schema[i].raw(); }
  }
