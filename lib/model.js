...
Model.prototype.init = function (obj, options) {
  ObjectProxy.prototype.init.call(this, obj, options);
  this.__modified = {};
};
...
Model.prototype.markAsModified = function (path, _newValue, _oldValue) {
      ...
      function (value) {
        ...
        if (!this.isModified()) {
          ...
          if (this._id) {
            ...
            $set = { $set: $set };
            debug('updating ' + JSON.stringify(where) + ' in ' + this.__definition.collection + ': ' + JSON.stringify($set));
            return this.__db.collection(this.__definition.collection)
              .then(function(collection) {
                var deferred = q.defer();
                collection.updateOne(where, $set, function (err, result) {
                  if (err) { return deferred.reject(err); }
                  debug('update into ' + this.__definition.collection + ' successful');
                  this.__modified = {};
            ...
            debug('inserting into ' + this.__definition.collection + ': ' + JSON.stringify(obj));
            return this.__db.collection(this.__definition.collection)
              .then(function(collection) {
                var deferred = q.defer();
                collection.insertOne(obj, function (err, result) {
                  if (err) { return deferred.reject(err); }
                  debug('insert into ' + this.__definition.collection + ' successful');
...
Model.prototype.remove = function () {
  var where = { _id: { $in: [this._id] }, _etag: this._etag };
  debug('removing ' + JSON.stringify(where) + ' in ' + this.__definition.collection);
  return this.__db.collection(this.__definition.collection)
    .then(function(collection) {
      var deferred = q.defer();
      collection.deleteOne(where, function (err, result) {
        if (err) { return deferred.reject(err); }
        debug('remove from ' + this.__definition.collection + ' successful');
        this._id = undefined;
...
Model.create = function (definition, superModel) {
  var schema = {}
  ...
  if (superModel) {
    ...
    if (!definition.collection) {
      definition.collection = superDef.collection;
    }
  ...
  Object.defineProperty(constr, '__name', {
    writable: false,
    enumerable: false,
  ...
  constr.extend = function (definition) {
    return Model.create(definition, constr);
  };

  constr.count = function (db, where, options) {
    where = _.assign({}, where, definition.where);
    options = options || {};
    for (var p in where) {
      var propertyDef = definition.properties[p];
      if (propertyDef) {
  ...
  switch (propertyDef.type.trim().toLowerCase()) {
    case 'array':
      if (!Array.isArray(value)) {
        value = [value];
      } else if (propertyDef.items){
        value = value.map(function (item, index) {
          return castProperty(name + '.' + index, item, propertyDef.items);
        });
      }
      break;
    case 'boolean':
