...
Model.extend = function (definition) {
  ...
  constr.extend = function (subDefinition) {
    if (definition.final) {
      throw new Error('A final model cannot be extended');
    }
    var newDef = {
      name: subDefinition.name,
      collection: subDefinition.collection,
      properties: _.assign({}, definition.properties, subDefinition.properties)
    };
    var subInit = subDefinition.init || subDefinition.initialize;
    var superInit = definition.init || definition.initialize;
    if (subInit && superInit) {
      newDef.init = function () {
        superInit.call(this);
        subInit.call(this);
      }
    } else {
      newDef.init = subInit || superInit;
    }
    return Model.extend(newDef);
  };
