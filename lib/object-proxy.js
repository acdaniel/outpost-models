    ...
    if (typeof path === 'object' && !util.isArray(path)) {
      ...
      if (path.length === 1) {
        ...
        if (type === 'virtual') {
          ...
          if (!options.silent) {
            this.emit('change', { path: path[0], currentPath: path[0], newValue: this.__obj[path[0]] });
          }
        } else {
...
Object.defineProperty(ObjectProxy.prototype, 'toObject', {
  writable: true,
    ...
    var obj = {};
    for (var p in this.__definition.properties) {
      if (!options.virtuals && this.__definition.properties[p].type === 'virtual') {
        continue;
      }
      var o = helpers.toObject(this[p]);
