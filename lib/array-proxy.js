...
ArrayProxy.prototype.toJSON = function (options) {
  var arr = [];
  for (var i = 0, l = this.__obj.length; i < l; i++) {
    arr[i] = helpers.toJson(this.__obj[i], options);
  }
  return arr;
};

ArrayProxy.prototype.toArray = ArrayProxy.prototype.toObject = function (options) {
  var arr = [];
  for (var i = 0, l = this.__obj.length; i < l; i++) {
    arr[i] = helpers.toObject(this.__obj[i], options);
  }
  return arr;
};
